...
class AGEMPlugin(SupervisedPlugin):
    ...
    def __init__(self, patterns_per_experience: int, sample_size: int, memory_filepath: str = "episodic_memory.pkl"):
        """
        :param patterns_per_experience: number of patterns per experience in the
            memory.
        :param sample_size: number of patterns in memory sample when computing
            reference gradient.
        :param memory_filepath: Path to save/load episodic memory.
        """
        super().__init__()
    ...
    def load_memory(self, filepath=None):
        ...
        if filepath is None:
            filepath = self.memory_filepath  # Use default path if not provided
        if os.path.exists(filepath):
            with open(filepath, 'rb') as f:
                self.buffers = pickle.load(f)
            print(f"Episodic memory loaded from {filepath}")
        else:
            print(f"No episodic memory found at {filepath}, starting fresh.")
    ...
    def save_memory(self,memory_path):
        ...
        print(f"Episodic memory saved to {memory_path}")
